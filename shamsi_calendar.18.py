...
def add_event(year, month, day):
    ...
    if event:
        ...
        events.setdefault(key, []).append((event, color_code))
        save_events()
        set_calendar()
...
def view_events(year, month, day):
    ...
    if day_events:
        msg = "\n".join(f"- {e[0]} (Color: {e[1]})" for e in day_events)
...
def edit_events(year, month, day):
    ...
    if not day_events:
        ...
        return
    list_text = "\n".join(f"{i+1}: {e[0]}" for i, e in enumerate(day_events))
    ...
    if choice:
        ...
        messagebox.showinfo("Deleted", f"Deleted: {deleted[0]}")
    else:
        new_text = simpledialog.askstring("Edit Event", "Edit event:", initialvalue=day_events[idx][0])
        if new_text:
            day_events[idx] = (new_text, day_events[idx][1])
    save_events()
    set_calendar()
...
def show_calendar(year, month):
    ...
    for day in range(1, get_days_in_month(year, month) + 1):
        ...
        add_tooltip(btn, "\n".join([e[0] for e in events.get(key, [])]) or "No events")
        col += 1
        if col > 6:
            col, row = 0, row + 1
...
def view_day_page(year, month, day):
    ...
    if day_events:
        for event in day_events:
            event_label = tk.Label(day_window, text=event[0], bg=theme["root_bg"], fg=theme["label_fg"], font=("Arial", 12))
    ...
    def add_event_button():
        event = simpledialog.askstring("Add Event", f"Enter event for {day}/{month}/{year}:")
        if event:
            events.setdefault(key, []).append((event, colorchooser.askcolor(title="Pick a color")[1]))
            save_events()
            set_calendar()
            day_window.destroy()
            view_day_page(year, month, day)
    ...
    add_event_btn = tk.Button(day_window, text="Add Event", command=add_event_button, bg=theme["button_bg"], fg=theme["button_fg"])
    add_event_btn.pack(pady=5)
    edit_event_btn = tk.Button(day_window, text="Edit Events", command=edit_event_button, bg=theme["button_bg"], fg=theme["button_fg"])
    edit_event_btn.pack(pady=5)
    delete_event_btn = tk.Button(day_window, text="Delete All Events", command=delete_event_button, bg=theme["button_bg"], fg=theme["button_fg"])
            ...
            with open(filepath, "r", encoding="utf-8") as f:
                ...
                for key, value in raw.items():
                    imported_events[tuple(map(int, key.split("-")))] = [
                        (event[0], event[1] if isinstance(event, (list, tuple)) and len(event) > 1 else '#cccccc')
                        for event in value
                    ]
